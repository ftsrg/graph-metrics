...
public class DC2Metrics implements EdgesFunctionWithVertexValue<IntValue, NullValue, String, Tuple2<IntValue, Double>> {
	...
	public List<String> getTriadCloserEdgeLabels(Edge<IntValue, String> edge1, Edge<IntValue, String> edge2, Vertex<IntValue, NullValue> vertex) {
		// ki kell v�lasztani az els� �l m�sik fel�t, nem tudjuk merre �llnak az �lek.
		...
		for (Edge<IntValue, String> edge : edgeList) {
			if (edge.getTarget().getValue() == edge2OtherVertexId.getValue() && !edge.getValue().equals(edge1.getValue())
					&& !edge.getValue().equals(edge2.getValue())) {
				labels.add(edge.f2);
			}
		}
		return labels;
